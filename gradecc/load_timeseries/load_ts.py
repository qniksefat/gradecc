...
def load_ts_cortex(subject: int, epoch: str) -> pd.DataFrame:
    """fMRI timeseries data
    Args:
        subject (int): subject number - or can be `dicom`
        epoch (str): time period during the task, such as `rest`, `baseline`, `early` learning
    Returns:
        pd.DataFrame: columns=regions, rows=time trials
    """
    # todo make subject invariant to int or str
    timeseries = _try_load_filenames(epoch, subject)
    return _window_timeseries(epoch, timeseries)
